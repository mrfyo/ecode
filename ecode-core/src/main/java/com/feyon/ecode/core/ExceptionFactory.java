 ...
 * 1. 如果指定类型实现了接口{@link EcodeAware}，该工厂类会再创建异常类后，会自动注入{@link Ecode}
...
public interface ExceptionFactory {
    ...
    /**
     * 设置 根异常类类型（实现了 {@link EcodeAware}）, 默认是使用 {@link EcodeException}
     * @param exType   根异常类类型， 同时实现 {@link EcodeAware}
     */
     ...
     * @return 代理异常类并且实现了 {@link EcodeAware}
     */
